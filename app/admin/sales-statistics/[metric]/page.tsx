...
const metricTitles: Record<string, string> = {
  "purchases-today": "Покупки сегодня",
  "purchases-month": "Покупки в этом месяце",
  "purchases-total": "Всего покупок",
  "earned-last-month": "Заработано в прошлом месяце",
  "earned-this-month": "Заработано в этом месяце",
  "earned-total": "Всего заработано",
  "net-last-month": "Чистая прибыль прошлый месяц",
  "net-this-month": "Чистая прибыль этот месяц",
  "net-total": "Всего чистой прибыли",
  "payout-sellers-last-month": "Выплаты продавцам прошлый месяц",
}
...
export default function MetricDetailPage() {
  ...
  const [comparisonRange, setComparisonRange] = useState({
    from: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000), // 60 дней назад
    to: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), // 30 дней назад
  })
  ...
  const fetchDetailedStats = async () => {
    try {
      ...
      const response = await fetch(`/api/sales-stats/detailed?${queryParams}`)

      if (!response.ok) {
        throw new Error("Не удалось загрузить детальную статистику")
      }
      ...
      toast({
        title: "Ошибка",
        description: "Не удалось загрузить детальную статистику",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const formatValue = (value: number) => {
    return metric.includes("earned") || metric.includes("net") || metric.includes("payout")
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="text-muted-foreground mb-4">Не удалось загрузить детальную статистику</p>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <BarChart3 className="h-8 w-8 text-primary" />
            <h1 className="text-3xl font-bold">{metricTitles[metric] || "Детальная статистика"}</h1>
          </div>
          <ThemeToggle />
        </div>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="text-4xl font-bold text-primary">{formatValue(stats.current_value)}</div>
          </CardContent>
        </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {formatValue(Math.max(...stats.historical_data.map((d) => d.value)))}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-600">
                    {formatValue(Math.min(...stats.historical_data.map((d) => d.value)))}
                  </div>
